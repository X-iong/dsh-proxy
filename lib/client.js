...
var NS = "dsh-proxy";
var CSS_TAG = "dsh-proxy/ProxyCard.module.css?v3";
var CSS = `
...
function statusLines(status) {
  if (status === void 0) return [{ warn: false, text: "\u5BBF\u4E3B\u72B6\u6001\uFF1A\u6682\u4E0D\u53EF\u8BFB" }];
  ...
  return h("li", { className: open ? "dsh-proxy-card dsh-proxy-card-open" : "dsh-proxy-card" }, [
    h(
      ...
      [
        ...
        status?.tunnel === "broken" ? h("span", { key: "warn", className: "dsh-proxy-warn" }, "\u96A7\u9053\u5F02\u5E38") : null,
    ...
    open ? h("div", { key: "body", className: "dsh-proxy-body" }, [
      ...
      h(
        ...
        statusLines(status).map(
          (line, index) => h("p", { key: index, className: line.warn ? "dsh-proxy-status-warn" : "dsh-proxy-status-line" }, line.text)
        )
      ),
