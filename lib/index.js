...
export const Config = Schema.object({
    ...
    host: Schema.string()
        .default('127.0.0.1')
        .description('代理服务器地址，例如 127.0.0.1（你的代理客户端监听的本机地址）。')
        .volatile(),
    port: Schema.natural()
        .max(65535)
        .default(7890)
        .description('代理服务器端口，例如 7890（你的代理客户端提供的 HTTP 代理端口）。')
        .volatile(),
 ...
 * means it is not (the proxy client is not running), so DSH should go direct.
 */
